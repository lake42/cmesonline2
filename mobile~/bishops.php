...
function bpage($hookup,$id)
{
	...
	if (in_array($row['rank_e'],$seniors)){
		$info.="<h2>Senior Bishop<br/> " . $row['bishop_name'] . "</h2>";
	}else{
		$info.="<h2>Bishop<br/> " . $row['bishop_name'] . "</h2>";
	}
	
	$info.="<div class='binfo'>" . $row['contact_info'] ;

	if ($row['website']){
		$info.="<br><a href='" . $row['website'] . "' target='_blank'>" . $row['website'] . "</a></p></div></div>";
	}
	else {
		$info.="</p></div>";
	}
