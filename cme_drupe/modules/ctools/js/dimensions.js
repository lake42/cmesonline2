...
(function ($) {
  ...
  $.fn.offset = function(options, returnObject) {
    var x = 0, y = 0, elem = this[0], parent = this[0], absparent=false, relparent=false, op, sl = 0, st = 0, options = $.extend({ margin: true, border: true, padding: false, scroll: true }, options || {});
    do {
      ...
      if ($.browser.mozilla || $.browser.msie) {
        ...
        if ($.css(parent, 'position') == 'absolute') absparent = true;
        // IE does not include the border on the body if an element is position static and without an absolute or relative parent
        if ($.css(parent, 'position') == 'relative') relparent = true;
      }
      ...
      if (parent && (parent.tagName.toLowerCase() == 'body' || parent.tagName.toLowerCase() == 'html')) {
        ...
        if ( ($.browser.mozilla && !absparent) ||
             ($.browser.msie && $.css(elem, 'position') == 'static' && (!relparent || !absparent)) ) {
          x += parseInt($.css(parent, 'borderLeftWidth')) || 0;
          y += parseInt($.css(parent, 'borderTopWidth'))  || 0;
        }
        break; // Exit the loop
